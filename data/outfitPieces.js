...
export const storeImage = async (
  ...
  checkRequiredFields(caption, link, outfitType, imageName, username);
  checkIfFieldsAreProperString(caption, link, outfitType, imageName, username);
...
export const getImage = async (imageName) => {
  checkRequiredFields(imageName);
  checkIfFieldsAreProperString(imageName);
  const outfitPiecesCollection = await outfitPieces();
...
export const getOutfitPiecesByUsername = async (username) => {
  ...
  const userOutfitPieces = outfitPiecesCollection
    ...
    .toArray();
  if (!userOutfitPieces)
    throw new Error(`Error getting outfit pieces for user ${username}`);
