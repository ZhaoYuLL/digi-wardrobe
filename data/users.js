...
const createUser = async (
  ...
  checkRequiredFields(firstName, lastName, username, password, age, email);
  checkIfFieldsAreProperString(
    firstName,
    lastName,
    username,
...
const updateUserInfo = async (id, userInfo) => {
  ...
  if (userInfo.username) {
    ...
    if (userExists)
      throw `User with username ${userInfo.username} already exists`;
...
export const addUserOutfitPiece = async (outfitPieceId, userId) => {
  ...
  if (!updateInfo)
    throw `Error updating user ${userId} closet with outfit piece ${outfitPieceId}`;
...
export const deleteUserOutfitPiece = async (outfitPieceId, userId) => {
  ...
  if (!updateInfo)
    throw `Error removing outfit piece ${outfitPieceId} from user ${userId}'s closet`;
...
const addUserFavorites = async (userId, fitpostId) => {
  ...
  if (!updateInfo)
    throw `Error updating user ${userId} favorites with fitpost ${outfitPieceId}`;
